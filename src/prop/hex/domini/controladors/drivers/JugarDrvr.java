...
public class JugarDrvr
{
	...
	{
		PartidaCtrl control_partida = new PartidaCtrl();
		...
		try
		{
			control_partida.inicialitzaPartida( 7, usuari_A, usuari_B, "Partida de Prova" );
		}
		catch ( Exception e )
		{
		...
		instanciaFinestra( control_partida );
	}
	...
	private static void instanciaFinestra( PartidaCtrl control_partida )
	{
		JFrame f = new JFrame( "Visor partida" );
		//f.setDefaultCloseOperation();
		f.add( new Finestra( control_partida ) );
		f.pack();
		f.setVisible( true );
	}
	...
	static class Finestra extends JPanel
	{
		...
		private PartidaCtrl control_partida;
		...
		public Finestra( PartidaCtrl control_partida )
		{
			tauler = ( TaulerHex ) control_partida.getPartidaActual().getTauler();
			jugador_a = ( UsuariHex ) control_partida.getPartidaActual().getJugadorA();
			jugador_b = ( UsuariHex ) control_partida.getPartidaActual().getJugadorB();
		...
		private void mouIA()
		{
			if ( control_partida.consultaEstatPartida() == EstatPartida.NO_FINALITZADA && !control_partida.esTornHuma() )
			{
				control_partida.executaMovimentIA();
			}
